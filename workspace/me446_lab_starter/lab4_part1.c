...
float Kpx = 50/2, Kpy = 50/2, Kpz = 100;
float Kdx = 2, Kdy = 2, Kdz = 2;
...
void lab(float theta1motor,float theta2motor,float theta3motor,float *tau1,float *tau2,float *tau3, int error) {
    ...
    Fx = Kpx*(xd-x_end) + Kdx*(velxd-velx);
    Fy = Kpy*(yd-y_end) + Kdy*(velyd-vely);
    Fz = Kpz*(zd-z_end) + Kdz*(velzd-velz);
    ...
    // Control Laws
    *tau1 = JT_11 * Fx + JT_12 * Fy + JT_13 * Fz + 0.6*u_fric1;
    *tau2 = JT_21 * Fx + JT_22 * Fy + JT_23 * Fz + 0.6*u_fric2;
    *tau3 = JT_31 * Fx + JT_32 * Fy + JT_33 * Fz + 0.6*u_fric3;
